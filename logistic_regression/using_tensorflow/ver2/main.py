...
def make_data(num_of_data1, num_of_data2, num_of_features,mean,std):
    ndata1 = num_of_data1
    ndata2 = num_of_data2
    nfeatures = num_of_features

    x1 = tf.random.normal(shape=[ndata1,nfeatures],mean=mean,stddev=std)
    x2 = tf.random.normal(shape=[ndata2, nfeatures], mean=mean, stddev=std)
    x = tf.experimental.numpy.vstack((x1,x2))

    y1 = tf.zeros(shape=[ndata1,nfeatures,1])
    y2 = tf.ones(shape=[ndata2,nfeatures,1])
    y = tf.experimental.numpy.vstack((y1,y2))

    plt.scatter(x[:,0],y[:,1],c=y[:,0])
    plt.show()
    return x, y


if __name__ == "__main__":
    X_train, Y_train = make_data(num_of_data1=1000,num_of_data2=1000,num_of_features=2,mean=[1,3],std=1.0)
    X_test, Y_test = make_data(num_of_data1=200,num_of_data2=200,num_of_features=2,mean=[3,1],std=1.0)
